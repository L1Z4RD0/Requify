...
CREATE TABLE USUARIOS (
    ...
    CONSTRAINT FK_USUARIO_POSEE FOREIGN KEY (ID_ROL)
...
CREATE TABLE MATERIALES (
    ...
    CONSTRAINT FK_MATERIAL_TIPO FOREIGN KEY (ID_TIPO_MATERIAL)
        REFERENCES TIPO_MATERIALES(ID_TIPO_MATERIAL)
);

-- ============================================
-- 4b. TABLA ITEMS_MATERIALES (Ítem físico)
-- ============================================
CREATE TABLE ITEMS_MATERIALES (
    ...
    CONSTRAINT FK_ITEM_MATERIAL FOREIGN KEY (ID_MATERIAL)
...
CREATE TABLE SOLICITUDES (
    ...
    CONSTRAINT FK_SOL_USUARIO FOREIGN KEY (ID_USUARIO)
        REFERENCES USUARIOS(ID_USUARIO),
    CONSTRAINT FK_SOL_ALUMNO FOREIGN KEY (ID_ALUMNO)
        REFERENCES ALUMNOS(ID_ALUMNO),
    CONSTRAINT FK_SOL_ASIGNATURA FOREIGN KEY (ID_ASIGNATURA)
...
CREATE TABLE DETALLE_SOLICITUD (
    ...
    CONSTRAINT FK_DET_SOL FOREIGN KEY (ID_SOLICITUD)
        REFERENCES SOLICITUDES(ID_SOLICITUD),
    CONSTRAINT FK_ITEM_ASOCIADO FOREIGN KEY (ID_ITEM)
        REFERENCES ITEMS_MATERIALES(ID_ITEM)
);
...
CREATE TABLE RECEPCIONES_MATERIAL (
    ...
    CONSTRAINT FK_REC_DET FOREIGN KEY (ID_DETALLE)
        REFERENCES DETALLE_SOLICITUD(ID_DETALLE),
...
CREATE TABLE HISTORIAL_ESTADOS (
    ...
    CONSTRAINT FK_HIST_SOL FOREIGN KEY (ID_SOLICITUD)
        REFERENCES SOLICITUDES(ID_SOLICITUD)
);
...
SELECT 1, CONCAT('TAB-', LPAD(n, 3, '0')), 1, 'Bodega A-1'
FROM (
    SELECT @row := @row + 1 AS n
    FROM (SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0
          UNION ALL SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0) t,
         (SELECT @row := 0) r
) AS seq
WHERE n <= 20;
...
-- Notebooks (100)
INSERT INTO ITEMS_MATERIALES (ID_MATERIAL, CODIGO_ITEM, ESTADO_ITEM, UBICACION_ITEM)
SELECT 2, CONCAT('NBK-', LPAD(n, 3, '0')), 1, 'Carro B-1'
FROM (
    SELECT @row2 := @row2 + 1 AS n
    FROM (
        SELECT 0 FROM
        (SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0) a,
        (SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0) b,
        (SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0) c
    ) t,
    (SELECT @row2 := 0) r
) seq
WHERE n <= 100;
...
-- Libros (50)
INSERT INTO ITEMS_MATERIALES (ID_MATERIAL, CODIGO_ITEM, ESTADO_ITEM, UBICACION_ITEM)
SELECT 3, CONCAT('LIB-', LPAD(n, 3, '0')), 1, 'Biblioteca'
FROM (
...
-- Material Deportivo (30)
INSERT INTO ITEMS_MATERIALES (ID_MATERIAL, CODIGO_ITEM, ESTADO_ITEM, UBICACION_ITEM)
SELECT 4, CONCAT('DEP-', LPAD(n, 3, '0')), 1, 'Gimnasio'
FROM (
    SELECT @row4 := @row4 + 1 AS n
    FROM (
        SELECT 0 FROM
        (SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0) a,
        (SELECT 0 UNION ALL SELECT 0 UNION ALL SELECT 0) b
    ) t,
    (SELECT @row4 := 0) r
) seq
WHERE n <= 30;
...
SET
  ...
  CANTIDAD_DISPONIBLE = (SELECT COUNT(*) FROM ITEMS_MATERIALES I WHERE I.ID_MATERIAL = M.ID_MATERIAL AND I.ESTADO_ITEM = 1);
...
VALUES (
    1, 'admin', 'Pedro', 'Fernández', 'admin@requify.cl',
